...
def getStxSubType():
    subprocess.call("blastn -query stx.fasta -db " + TOOL_DIR + "/data/stx -task blastn -evalue 0.001 -out shigatoxin -outfmt '6 std sallseqid score nident positive gaps ppos qframe sframe qseq sseq qlen slen salltitles' -num_threads 8 -strand both -dust yes -max_target_seqs 1 -perc_identity 95.0", shell=True)
...
#process fastq files from Illumina data
if args.input =='illumina':
    for file in glob.glob("*_R1*"):
        ...
        if p2_file in find_p2:
            ...
            # ASSEMBLY
            subprocess.call(
            "perl " + TOOL_DIR + "/scripts/spades.pl " + sample + "_spades_contigs " + sample + "_spades_contig_stats " + sample + "_spades_scaffolds " + sample + "_spades_scaffold_stats " + sample + "_spades_log NODE spades.py --disable-gzip-output --isolate --pe1-ff --pe1-1"+ " trimmed_" + sample + "_1.fq "+ "--pe1-2 trimmed_" + sample + "_2.fq",
            shell=True)
            subprocess.call(
            "perl " + TOOL_DIR + "/scripts/filter_spades_repeats.pl -i " + sample + "_spades_contigs -t " + sample + "_spades_contig_stats -c 0.33 -r 1.75 -l 1000 -o " + sample + "_output_with_repeats -u " + sample + "_contigs -n " + sample + "_repeat_sequences_only -e 5000 -f " + sample + "_discarded_sequences -s " + sample + "_summary",
            shell=True)
    ...
    # SHIGATOXIN TYPER
        subprocess.call(TOOL_DIR + "/scripts/stx_subtype_pe.sh " + TOOL_DIR + " trimmed_" + sample + "_1.fq " + "trimmed_" + sample + "_2.fq " + sample + "_contigs",
            shell=True)
        results_stx = []
    ...
    # SEROTYPER O&H
        subprocess.call(TOOL_DIR + "/scripts/serotype.sh " + TOOL_DIR + " y trimmed_" + sample + "_1.fq "+ "trimmed_" + sample + "_2.fq " + sample + "_contigs",
        shell=True)
        results_o = []
        results_h = []
    ...
    # AMRGENES, abricate
        subprocess.call("abricate -db ncbi "+ sample +"_contigs > amr_abricate_results.txt", shell=True)
    ...
    # chewBBACA
        ...
        subprocess.call("chewBBACA.py AlleleCall -i ./chewbbca/ -g  /ariesdb/database/INNUENDO/schema_chewBBACA_cgMLST_V4 -o chewbacca_results/ --cpu 6 --bsr 0.6 --ptf /afs/galaxy/tools/INNUENDO/chewBBACA_2.0/TrainingFiles4Prodigal/trained_eColi.trn --fr", shell=True)
...
#process fastq files from Ion Torrent data
if args.input=='ion':
    ...
    for file in list_ngs_data:
        ...
        # ASSEMBLY
        subprocess.call(
        "perl " + TOOL_DIR + "/scripts/spades.pl " + sample + "_spades_contigs " + sample + "_spades_contig_stats " + sample + "_spades_scaffolds " + sample + "_spades_scaffold_stats " + sample + "_spades_log NODE spades.py --disable-gzip-output --isolate --iontorrent -s trimmed_" + sample + ".fq",
        shell=True)
        subprocess.call(
        "perl " + TOOL_DIR + "/scripts/filter_spades_repeats.pl -i " + sample + "_spades_contigs -t " + sample + "_spades_contig_stats -c 0.33 -r 1.75 -l 1000 -o " + sample + "_output_with_repeats -u " + sample + "_contigs -n " + sample + "_repeat_sequences_only -e 5000 -f " + sample + "_discarded_sequences -s " + sample + "_summary",
        shell=True)
    ...
    # SHIGATOXIN TYPER
        subprocess.call(TOOL_DIR + "/scripts/stx_subtype_se.sh " + TOOL_DIR + " trimmed_" + sample + ".fq " + sample + "_contigs",
        shell=True)
        results_stx = []
        results_stx.append(getStxSubType())
        stx=open("shigatoxin_results.txt", "w")
    ...
    # SEROTYPER O&H
        subprocess.call(TOOL_DIR + "/scripts/serotype.sh " + TOOL_DIR + " n trimmed_" + sample + ".fq xxx " + sample + "_contigs",
        shell=True)
        results_o = []
        results_h = []
        results_o.append(getSeroGroup('serogroup_O', "O"))
    ...
    #chewBBACA
        ...
        subprocess.call("mkdir chewbbca", shell=True)
        subprocess.call("cp " + sample + "_contigs ./chewbbca/" + sample + "_contigs.fasta",shell=True)
        subprocess.call("python "+chewbbca+"chewBBACA.py AlleleCall -i ./chewbbca/ -g  /ariesdb/database/INNUENDO/schema_chewBBACA_cgMLST_V4 -o chewbacca_results/ --cpu 6 --bsr 0.6 --ptf /afs/galaxy/tools/INNUENDO/chewBBACA_2.0/TrainingFiles4Prodigal/trained_eColi.trn --fr", shell=True)
        subprocess.call("cp chewbacca_results/results_*/results_statistics.tsv ./chewbbca_results.tsv", shell=True)
        subprocess.call("cp chewbacca_results/results_*/results_alleles.tsv ./results_alleles.tsv", shell=True)
        subprocess.call("cp chewbacca_results/results_*/RepeatedLoci.txt ./RepeatedLoci.txt", shell=True)
        os.chdir('../')

subprocess.call("python " + TOOL_DIR + "/discover_results.py", shell=True)
